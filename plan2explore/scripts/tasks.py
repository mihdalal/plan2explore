...
def cheetah_run_back(config, params):
  ...
  state_components = ['position', 'velocity']
  env_ctor = tools.bind(
      _dm_control_env, 'cheetah', 'run_back', config, params, action_repeat)
  return Task('cheetah_run_back', env_ctor, state_components)

def cheetah_flip_forward(config, params):
  action_repeat = params.get('action_repeat', 2)
  state_components = ['position', 'velocity']
  env_ctor = tools.bind(
      _dm_control_env, 'cheetah', 'flip_forward', config, params, action_repeat)
  return Task('cheetah_flip_forward', env_ctor, state_components)

def cheetah_flip_backward(config, params):
  action_repeat = params.get('action_repeat', 2)
  state_components = ['position', 'velocity']
  env_ctor = tools.bind(
      _dm_control_env, 'cheetah', 'flip_backward', config, params, action_repeat)
  return Task('cheetah_flip_forward', env_ctor, state_components)
################################################
...
############
def walker_jump(config, params):
  action_repeat = params.get('action_repeat', 2)
  state_components = ['height', 'orientations', 'velocity']
  env_ctor = tools.bind(
      _dm_control_env, 'walker', 'jump', config, params, action_repeat)
  return Task('walker_jump', env_ctor, state_components)

def walker_walk_back(config, params):
  action_repeat = params.get('action_repeat', 2)
  state_components = ['height', 'orientations', 'velocity']
  env_ctor = tools.bind(
      _dm_control_env, 'walker', 'walk_back', config, params, action_repeat)
  return Task('walker_walk_back', env_ctor, state_components)
