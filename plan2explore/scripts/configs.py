...
from plan2explore import models
from plan2explore import networks
from plan2explore import tools
from plan2explore.scripts import tasks as tasks_lib
from plan2explore.scripts import objectives as objectives_lib
...
DEFAULTS = dict(
    ...
    random=dict(
        train_planner='random_sample',
        test_planner='policy_mode',
        planner_objective='reward_value',
        action_head=True,
        value_head=True,
        train_action_noise=1.0,
    ),
    disagree=dict(
        train_planner='curious_sample',
        test_planner='policy_mode',
        planner_objective='curious_reward_value',
        action_head=True,
        curious_action_head=True,
        value_head=True,
        curious_value_head=True,
        train_action_noise=0.3,
    ),
    prediction_error=dict(
        train_planner='curious_sample',
        test_planner='policy_mode',
        planner_objective='icm_reward_value',
        action_head=True,
        curious_action_head=True,
        value_head=True,
        curious_value_head=True,
...
def _model_components(config, params):
  ...
  if 'prediction_error' in params.get('defaults', ['dreamer']):
      config.gradient_heads.append('reward_int')
  ...
  config.vanilla_curious_run = False
  config.curious_run = False
  config.combination_run = False
  config.adaptation = params.get('adaptation', False)
  config.encoder_feature_shape = params.get('encoder_feature_shape', 1024)

  if 'prediction_error' in params.get('defaults', ['dreamer']):
      config.vanilla_curious_run = True
  ...
  if 'disagree' in params.get('defaults', ['dreamer']):
      ...
      config.extrinsic_train_frequency = params.get('extrinsic_train_frequency', 1)
  ...
  if params.get('curious_value_head', False):
    config.heads.curious_value = tools.bind(
        config.head_network,
        num_layers=params.get('value_layers', 3),
        data_shape=[],
        dist=params.get('value_dist', 'normal'))
  ...
  if params.get('curious_action_head', False):
    config.heads.curious_action = tools.bind(
        config.head_network,
        num_layers=params.get('action_layers', 4),
        mean_activation=ACTIVATIONS[
            params.get('action_mean_activation', 'none')],
  ...
  hidden_size = params.get('model_size', 400)
  state_size = params.get('state_size', 60)
...
def _tasks(config, params):
  ...
  for name in train_tasks[0].state_components + ['reward', 'pcont']:
    ...
    if config.vanilla_curious_run:
        if name=='reward':
            config.heads['reward_int'] = tools.bind(config.head_network, **kwargs)
...
def _loss_functions(config, params):
  ...
  if params.get('curious_value_head', False):
      config.optimizers.curious_value = config.optimizers.default.copy(
          learning_rate=params.get('curious_value_lr', 8e-5),
          clipping=params.get('value_gradient_clipping', 100.0))
  ...
  if params.get('curious_action_head', False):
      config.optimizers.curious_action = config.optimizers.default.copy(
          learning_rate=params.get('curious_action_lr', 8e-5),
          clipping=params.get('action_gradient_clipping', 100.0))
...
def _active_collection(tasks, collects, defaults, config, params):
  ...
  for task in tasks:
    for user_collect in collects:
      ...
      adaptation_condition = (collect.prefix=='train' and config.curious_run and config.adaptation) or (collect.prefix=='train' and config.random_run and config.adaptation) or (collect.prefix=='train' and config.vanilla_curious_run and config.adaptation)
      if adaptation_condition:
          collect.secondary_planner = _define_planner(
              collect.secondary_planner, collect.horizon, config, params)
          collect.secondary_objective = tools.bind(
              getattr(objectives_lib, collect.secondary_objective), params=params)
...
def _define_planner(planner, horizon, config, params):
  if planner == 'cem':
    ...
    planner_fn = tools.bind(
        ...
        strategy='mode',
        config=config)
  elif planner == 'curious_sample':
    planner_fn = tools.bind(
        control.planning.action_head_policy,
        strategy='curious_sample',
        config=config)
  elif planner == 'random_sample':
    planner_fn = tools.bind(
        control.planning.action_head_policy,
